...
import UpdateProductDialog from "@/components/EditSection/Edit";
    ...
    useEffect(() => {
        const getProducts = async () => {
            try {
                ...
                const allProducts = Object.entries(snapshot.val() as Record<string, any>).map(([id, value]) => ({
                    id,
        ...
        <>
            {
                loading ?
                    <div>
                        <h1 className="text-3xl font-bold text-white flex items-center justify-center">  Loading...</h1>
                    </div>
                    :
                    products.length > 0 ?
                        <>
